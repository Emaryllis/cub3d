 ...
 */
static int	realloc_bitmask_cap(t_map *map, t_p_map *p_map)
{
	char	*new_ptr;
	t_tile	*new_row;
	size_t	old_bytes;
	...
	old_bytes = (map->cap.x >> TILE_BITS) + 1;
	new_bytes = ((map->cap.x * 2) >> TILE_BITS) + 1;
	...
	new_row = resize_buffer(map->grid[p_map->curr.y],
			&map->cap.x, sizeof(t_tile));
	if (!new_row)
		return (parse_error(EBC_RESIZE_ERR));
	map->grid[p_map->curr.y] = new_row;
	return (0);
}
...
int	read_char(t_map *map, t_p_map *p_map)
{
	...
	if (c == '\n')
		return (c);
	if (p_map->curr.x >= map->cap.x
		&& realloc_bitmask_cap(map, p_map) == -1)
		return (-1);
	return (c);
}
...
int	next_row(t_map *map, t_p_map *p_map, size_t *grid_cap)
{
	...
	{
		new_grid = resize_buffer(map->grid, grid_cap, sizeof(t_tile *));
		if (!new_grid)
	...
	p_map->curr.x = 0;
	map->grid[p_map->curr.y] = malloc(sizeof(t_tile) * map->cap.x);
	if (!map->grid[p_map->curr.y])
		return (parse_error(NEW_ROW_MALLOC_ERR));
	return (migrate_bitmask_states(p_map), read_char(map, p_map));
}
